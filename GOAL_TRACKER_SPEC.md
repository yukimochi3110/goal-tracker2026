...
#### 年代ごとの縦リスト
- 年代の見出し（例「41-45歳」）ごとに、その年代のやりたいことを並べる
- 見出しは帯として見えるように背景色 `--tb-head-bg` と下線を付ける
- 現在の年齢が含まれる年代は見出しの背景をより濃い `--tb-head-now-bg` にし、
  文字色を `--tb-head-now-tx` に変えて「いま」バッジを表示
...
| 状態 | 背景色（ライト） | 背景色（ダーク） |
|------|------------------|------------------|
| 達成 | 黄色 `--tb-done-bg` #FCF8C4 | #34321A |
| 進行中 | 白（カードの地色のまま） | カードの地色のまま |
| 中止 | 灰色 `--tb-stop-bg` #E9EDEA | #232823 |

達成年齢の文字色は `--tb-done-tx`（ライト #7C6C22 / ダーク #CBC183）。黄色い背景の上で読めるよう専用に用意する。
